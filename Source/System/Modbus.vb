...
Namespace Ports
  Public Class Modbus
    ...
    Property ClickIOSingles As Boolean = False
          ...
          Case Func.ReadInputRegisters, Func.ReadHoldingRegisters
            ...
            Else
              ...
              If singleArray IsNot Nothing Then
                For i As Integer = 0 To count - 1
                  ' Need first to swap around the two bytes inside each word because Modbus is backwards-way-round with that
                  ' ...for Click IO
                  If ClickIOSingles Then
                    Dim ofs = 3 + 4 * i
                    singleArray(i + 1) = System.BitConverter.ToSingle(New Byte() {rx_(ofs + 1), rx_(ofs + 0), rx_(ofs + 3), rx_(ofs + 2)}, 0)
                  Else
    ...
    Public Function Write(ByVal slaveAddress As Integer, ByVal firstRegister As Integer, ByVal values As Array, ByVal writeMode As WriteMode) As Result
      If state_ = State.Idle Then
        ...
        If writeMode = Ports.WriteMode.Optimised AndAlso _
           writeOptimisation_.RecentlyWrote(values, slaveAddress, firstRegister) Then Return Result.OK
        ...
        If (firstRegister >= 10001 AndAlso firstRegister <= 19999) OrElse (firstRegister = 26385) Then     ' 26385 for Watchdog Bit (C1 = modbus 16385)
        ...
        ElseIf firstRegister >= 40001 Then
          ...
          Else
            ...
            If singleArray IsNot Nothing Then
              For i As Integer = 0 To count - 1
                If ClickIOSingles Then
                  Dim sb = System.BitConverter.GetBytes(singleArray(i + 1))  ' bytes for the Single
                  Dim ofs = i * 4
                  data(ofs + 0) = sb(1)
                  data(ofs + 1) = sb(0)
                  data(ofs + 2) = sb(3)
      ...
      If state_ <> State.Idle Then Return Result.Busy ' not yet 
      ' TODO: check the reply looks appropriate, although it has no other data for us
      Return result_
    End Function
